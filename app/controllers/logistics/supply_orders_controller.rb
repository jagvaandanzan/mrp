class Logistics::SupplyOrdersController < Logistics::BaseController
  before_action :set_supply_order_item, only: [:edit, :show, :update, :destroy]
  ...
  def new
    @supply_order_item = ProductSupplyOrderItem.new
    @supply_order_item.ordered_date = Time.current
    @supply_order_item.code = ApplicationController.helpers.get_code(ProductSupplyOrderItem.last)
    @supply_order_item.product_supply_order_items << ProductSupplyOrderItem.new
  end

  def create
    @supply_order_item = ProductSupplyOrderItem.new(supply_order_item_params)
    if @supply_order_item.save
      flash[:success] = t('alert.saved_successfully')
      redirect_to :action => 'show', id: @supply_order_item.id
    else
      logger.debug(@supply_order_item.errors.full_messages)
      render 'new'
    end
  end

  def edit
    @supply_order_item.tab_index = params[:tab_index] if params[:tab_index].present?
    @supply_order_item.product_supply_order_items.each do |item|
  ...
  def update
    ...
    if @supply_order_item.save
      ...
      if @supply_order_item.product_supply_order_items.count > 0
  ...
  def form_feature
    ...
    if @order_item.save
      ...
      flash[:success] = t('alert.info_updated')
      supply_order_item = @order_item.supply_order_item
      if supply_order_item.product_supply_order_items.count == @order_item.tab_index.to_i
        redirect_to action: 'index'
      else
  ...
  def supply_order_item_params
    params.require(:supply_order_item).permit(:code, :ordered_date, :supplier_id, :payment, :exchange, :exchange_value,
