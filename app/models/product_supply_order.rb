class ProductSupplyOrder < ApplicationRecord
  ...
  scope :search, ->(start, finish, supply_code, product_name, order_type, is_equal) {
    ...
    items = items.joins(:products)
                .where('products.code LIKE :value OR products.n_name LIKE :value OR products.c_name LIKE :value', value: "%#{product_name}%")
                .group("product_supply_orders.id") if product_name.present?
